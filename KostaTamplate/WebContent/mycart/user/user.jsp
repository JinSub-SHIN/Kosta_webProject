...
$(function(){
	...
	$("[class=settings]").click(function(){
		location.href="selectByIdCustomer?userId="+$(this).attr('value');
	});//end event
	
	$("#addUser").click(function(){
		//관리자가 관리자 정보(admin person) 추가
	});
});
</script>
<% List<Customer> customerList = (List<Customer>)request.getAttribute("list");  %>
<%=customerList %> <br>
<c:forEach items="<%=customerList %>" var="customer" varStatus="state">
${customer} / ${state.index} / ${state.count}<br>
</c:forEach>
        ...
        <div class="table-wrapper">
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>${state.count}</td>
                        <td><a href="#" class="avatar" alt="Avatar">${customer.name}</a></td>
                        ...
                        <td>
							<c:choose>
								...
								<c:when test="${customer.status == 4}"> <!-- 탈퇴 회원 -->
									User
								</c:when>
							</c:choose>
						</td>
						<td>
							<c:choose>
								<c:when test="${customer.status == 4}">
									<span class="status text-unsuccess">&bull;</span> Inactive
								</c:when>
								<c:otherwise>
						...
						<td>
							<a href="#" value="${customer.id}" class="settings" title="Settings" data-toggle="tooltip"><i class="material-icons">&#xE8B8;</i></a>
